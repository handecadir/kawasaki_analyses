...
@app.cell
def _(hl):
    filtered_mis = hl.read_matrix_table('missense.mt')
    return (filtered_mis,)
...
@app.cell
def _(filtered_mis, hl, kegg_genes):
    kegg_genes_final = hl.literal(set(kegg_genes))

    kegg_genes_mis = filtered_mis.filter_rows(
        kegg_genes_final.contains(filtered_mis.gene_symbol)
    )

    print("Original variant number:", filtered_mis.count_rows())
    print("Total pathway variant number:", kegg_genes_mis.count_rows())

    kegg_genes_mis.write("kegg_mis_variants.mt", overwrite=True)
...
def _(hl, kegg_genes_mis):
    #familyhistorychd
    hl.export_plink(
        dataset=kegg_genes_mis,
        output='plink/kegg_genes_mis_famhis_chd',          
        pheno=kegg_genes_mis.pheno.Family_history_of_CHD_status
    )
    ...
    hl.export_plink(
        dataset=kegg_genes_mis,
        output='plink/kegg_genes_mis_caa',         
        pheno=kegg_genes_mis.pheno.CAA_status
    )

    # Diagnostic_Age_Status
    hl.export_plink(
        dataset=kegg_genes_mis,
        output='plink/kegg_genes_mis_diagnostic_age',          
        pheno=kegg_genes_mis.pheno.Diagnostic_Age_Status
    )

    #Sequelae_Status
    hl.export_plink(
        dataset=kegg_genes_mis,
        output='plink/kegg_genes_mis_Sequelae_Status',          
        pheno=kegg_genes_mis.pheno.Sequelae_Status
    )

    #Family_History_Status
    hl.export_plink(
        dataset=kegg_genes_mis,
        output='plink/kegg_genes_mis_Family_History_Status',          
        pheno=kegg_genes_mis.pheno.Family_History_Status
    )
    ...
    hl.export_plink(
        dataset=kegg_genes_mis,
        output='plink/kegg_genes_mis_Consanguineous_marriage_status',         
        pheno=kegg_genes_mis.pheno.Consanguineous_marriage_status
    )

    #Degree_of_CM
    hl.export_plink(
        dataset=kegg_genes_mis,
        output='plink/kegg_genes_mis_Degree_of_CM',         
        pheno=kegg_genes_mis.pheno.Degree_of_CM
    )

    #KD_in_siblings_status
    hl.export_plink(
        dataset=kegg_genes_mis,
        output='plink/kegg_genes_mis_KD_in_siblings_status',          
        pheno=kegg_genes_mis.pheno.KD_in_siblings_status
    )
    return
...
@app.cell
def _(hl, kegg_genes_mis):
    kegg_genes_mis_csq2 = kegg_genes_mis.annotate_cols(
        case_numeric = hl.case()
            .when(kegg_genes_mis.case_control_status == "case", 1)
            .when(kegg_genes_mis.case_control_status == "control", 0)
            .or_missing()  
    )

    hl.export_plink(
        dataset=kegg_genes_mis_csq2,
        output='plink/kegg_genes_mis_case_control',
...
def _(hl, kegg_genes_mis):
    ...
    hl.export_plink(
        dataset=kegg_genes_mis,
        output='plink/kegg_genes_mis_Age_at_diagnosis_years',          
        pheno=kegg_genes_mis.pheno.Age_at_diagnosis_years
    )
    return
...
@app.cell
def _(hl, kegg_genes_mis):
    kegg_genes_mis_csq3 = kegg_genes_mis.annotate_cols(
        Sex_numeric = hl.case()
            .when(kegg_genes_mis.pheno.Sex == "Male", 1)
            .when(kegg_genes_mis.pheno.Sex == "Female", 2)
            .default(0)
    )
...
@app.cell
def _(hl, kegg_genes_mis):
    kegg_genes_mis_csq = kegg_genes_mis.annotate_cols(
        Class_numeric = hl.case()
            .when(kegg_genes_mis.pheno.Class == "a", 1)
            .when(kegg_genes_mis.pheno.Class == "b", 2)
            .when(kegg_genes_mis.pheno.Class == "c", 3)
            .when(kegg_genes_mis.pheno.Class == "d", 4)
            .when(kegg_genes_mis.pheno.Class == "ab", 5)
            .when(kegg_genes_mis.pheno.Class == "ac", 6)
            .when(kegg_genes_mis.pheno.Class == "ad", 7)
            .when(kegg_genes_mis.pheno.Class == "bc", 8)
            .when(kegg_genes_mis.pheno.Class == "bd", 9)
            .when(kegg_genes_mis.pheno.Class == "cd", 10)
            .when(kegg_genes_mis.pheno.Class == "abc", 11)
            .when(kegg_genes_mis.pheno.Class == "abd", 12)
            .when(kegg_genes_mis.pheno.Class == "bcd", 13)
            .when(kegg_genes_mis.pheno.Class == "abcd", 14)
    ...
    hl.export_plink(
        ...
        pheno=kegg_genes_mis_csq.Class_numeric
    )
    return

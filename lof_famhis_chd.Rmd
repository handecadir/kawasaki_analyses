...
if (nrow(all_pathways_sorted) > 0) {
  ...
  plot_all_pathways <- ggplot(all_pathways_sorted, aes(x = SetID_ordered, y = P.value)) +
    ...
    coord_flip() + 
    labs(
      title = "Pathway p-Values: LoF Family History CHD",
      x = "Pathway",
      y = "p-Value"
    ) +
  ...
  ggsave("skat_lof_famhis_chd.png", plot = plot_all_pathways, width = 10, height = max(7, nrow(all_pathways_sorted) * 0.15))
...
if(nrow(significant_pathways) > 0) {
  ...
  output_file <- "./kawasaki/plink/significant_pathways_snps_lof_famhis_chd.csv"

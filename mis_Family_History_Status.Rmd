...
if (nrow(all_pathways_sorted) > 0) {
  ...
  plot_all_pathways <- ggplot(all_pathways_sorted, aes(x = SetID_ordered, y = P.value)) +
    ...
    coord_flip() + 
    labs(
      title = "Pathway p-Values:Missense Family History Status",
      x = "Pathway",
      y = "p-Value"
    ) +
  ...
  ggsave("skat_mis_Family_History_Status.png", plot = plot_all_pathways, width = 10, height = max(7, nrow(all_pathways_sorted) * 0.15))
...
if(nrow(significant_pathways) > 0) {
  ...
  output_file <- "./kawasaki/plink/significant_pathways_snps_mis_Family_History_Status.csv"
